...
class PFConnect(nn.Module):
    ...
    def forward(self, x):
        ...
        _f1 = resize(_f1, size=f1.size()[2:],mode='linear',align_corners=False)

        # 融入全局信息
        global_weight = self.linear1(_f4)
        _f3_n = global_weight + _f3
        _f2_n = global_weight + _f2
        _f1_n = global_weight + _f1
